...
If you wrapped your value in `context()`, the factory runs once per connection, so each peer can get its own value, see [connections](/guides/connections/#a-different-value-per-peer).
...
What the context contains on each transport is covered in [connections](/guides/connections/#what-is-in-the-context).
...
const api = {}
// ---cut---
expose(api, { transport: globalThis, name: 'worker', remoteName: 'page' })
```
...
type WorkerApi = {}
// ---cut---
const remote = await expose<WorkerApi>(api, { transport: worker, name: 'page', remoteName: 'worker' })
```
...
Your value only travels in the addressed messages that follow, which do respect `origin`. See [transports](/guides/transports/#iframe) for a worked iframe setup.
